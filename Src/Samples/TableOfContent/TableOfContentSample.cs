...
namespace Xceed.Words.NET.Examples
{
  public class TableOfContentSample
  {
    ...
    {
#if !OPEN_SOURCE
      Console.WriteLine( "\tUpdateTableOfContent" );

      // Creates a document
      using( var document = DocX.Load( TableOfContentSample.TableOfContentSampleResourcesDirectory + @"TOCDocument.docx" ) )
      {
        ...
        if( p != null )
        {
          ...
          var newParagraph = p.InsertParagraphBeforeSelf( "A new Header" ).Color( System.Drawing.Color.Red ).SpacingAfter( 25d );
          newParagraph.StyleId = "Heading1";
          // Add another paragraph with a Page break.
          newParagraph.InsertParagraphAfterSelf( "New set of data." ).InsertPageBreakAfterSelf();

          // Force an update of the Table of Content.
          document.UpdateFields();
        }

        document.SaveAs( TableOfContentSample.TableOfContentSampleOutputDirectory + @"UpdateTableOfContent.docx" );
        Console.WriteLine( "\tCreated: UpdateTableOfContent.docx\n" );
      }
#else
